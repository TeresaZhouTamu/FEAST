...
def load_esmc_model(device='cuda'):
    ...
    model = ESMC.from_pretrained("esmc_600m", device=torch.device(device)).to(torch.float32)
    return model, tokenizer

# def load_esmc_model(device='cuda'):
#     # get_esmc_model_tokenizers() often returns a dict or a namespace
#     tokenizers = get_esmc_model_tokenizers()
    
#     # Check if 'tokenizers' is a dict; if so, pick the 'esmc_300m' one
#     if isinstance(tokenizers, dict):
#         tokenizer = tokenizers.get("esmc_300m") or list(tokenizers.values())[0]
#     else:
#         tokenizer = tokenizers

#     model = ESMC.from_pretrained("esmc_300m", device=torch.device(device))
#     return model.to(torch.float32), tokenizer
