...
if __name__ == "__main__":
    ...
    # Calculate Size Normalization Scalar
    n_pos = len(pos_seqs)
    n_neg = len(neg_seqs)
    effective_n = math.sqrt(n_pos * n_neg)
    
    # Apply sqrt(N) scaling
    size_scalar = math.sqrt(effective_n)
    print(f"Client Sample Profile: Pos={n_pos}, Neg={n_neg} -> Applying Size Scalar: {size_scalar:.4f}")
    ...
    print("Computing size-normalized mean steering vectors...")
    ...
    for i in range(n_layers):
        ...
        pos_steering_vectors.append(pos_vec * size_scalar)
        neg_steering_vectors.append(neg_vec * size_scalar)
    ...
    output_path = f"{args.save_folder}/ESMC_{args.property}_scaled_steering_vectors.pt"
    
    torch.save((pos_steering_vectors, neg_steering_vectors), output_path)
    print(f"Size-normalized steering vectors saved to: {output_path}")