...
if __name__ == "__main__":
    ...
    parser.add_argument('--sv_from', type=str, default="weighted_vectors", help="Folder path where steering vectors are saved.")
    ...
    # Optimize Negative samples (0) -> Positive (1)
    ...
    print(f"Loading steering vectors from: {args.sv_from}")
    
    if not os.path.exists(args.sv_from):
        raise FileNotFoundError(f"Steering vector file not found at: {args.sv_from}")

    pos_steering_vectors, neg_steering_vectors = torch.load(args.sv_from, map_location=args.device)
    
    # Compute Difference Vector (Pos - Neg)
